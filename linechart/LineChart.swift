...
//  MagicBricks
//
//  Created by Mayank Gupta on 13/11/15.
...
public class LineChart: UIView {
    ...
    */
    ...
    private var selectedVerticalMarker : UIView?
    ...
    override public func drawRect(rect: CGRect) {
        ...
        self.drawingWidth = self.bounds.width - (x.axis.inset) - x.axis.margin
    ...
    private func handleTouchEvents(touches: NSSet!, event: UIEvent) {
        ...
        //Draw current selected vertical line
        if selectedVerticalMarker == nil{
            selectedVerticalMarker = UIView(frame: CGRectMake(0, 0, 0.5, 100))
            selectedVerticalMarker!.backgroundColor = UIColor.blackColor()
            self.addSubview(selectedVerticalMarker!)
        }
    ...
    private func highlightDataPoints(index: Int) {
        ...
        for (lineIndex, dotsData) in dotsDataStore.enumerate() {
            ...
            if index < 0 {
                ...
                dot = dotsData[index]
                xCoordinate = dot.position.x
            }
            dot.backgroundColor = Helpers.lightenUIColor(colors[lineIndex]).CGColor
        }
        selectedVerticalMarker!.frame = CGRectMake(xCoordinate, y.axis.inset, 0.5, self.bounds.height - 80)
    }
    ...
    private func drawAxes() {
        ...
        path.addLineToPoint(CGPoint(x: width - x.axis.margin, y: y0))
        path.stroke()
        // draw y-axis
    ...
    private func getMaximumValue() -> CGFloat {
        ...
        {
            if max % 10 != 0{
                return ceil(max/10.0)*10
            }
        }
        else if max < 1000
        {
            if max % 100 != 0{
                return ceil(max/100.0)*100
            }
        }
        else
        {
            if max % 1000 != 0{
                return ceil(max/1000.0)*1000
            }
        }
    ...
    private func getMinimumValue() -> CGFloat {
        var min: CGFloat? = dataStore[0].filter { $0 != 0 }.minElement()
        for data in dataStore {
            let newMin = data.filter { $0 != 0 }.minElement()
            if (newMin != nil) && ((min == nil) || (newMin < min)){
                min = newMin
            }
        }
        ...
        {
            if min! % 10 != 0{
                return CGFloat((Int(min!)/10)*10)
            }
        }
        else if min < 1000
        {
            if min! % 100 != 0{
                return CGFloat((Int(min!)/100)*100)
            }
        }
        else
        {
            if min! % 1000 != 0{
                return CGFloat((Int(min!)/1000)*1000)
            }
        }
    ...
    private func drawXGrid() {
        ...
        let x1 = self.bounds.width - x.axis.margin
    ...
    private func drawYGrid() {
        ...
        let x2: CGFloat = self.bounds.width - x.axis.margin
        var y1: CGFloat
    ...
    private func drawXLabels() {
        ...
        let width = x.scale(step) + 5 //Addtional width to accomodate for year
